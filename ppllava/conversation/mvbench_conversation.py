    ...
    with torch.no_grad():
        seg_tokens = [
            model.llama_tokenizer(
                seg, return_tensors="pt", add_special_tokens=i == 0).to("npu:0").input_ids
...
def get_context_emb_sim(conv, model, img_list, answer_prompt=None):
    ...
    with torch.no_grad():
        seg_tokens = model.llama_tokenizer(
                [question], return_tensors="pt", add_special_tokens=0 == 0).to("npu:0")
...
def answer(conv, model, img_list, ask_simple=False, do_sample=True, max_new_tokens=200, num_beams=1, min_length=1, top_p=0.9,
    ...
    stop_words_ids = [
        torch.tensor([835]).to("npu:0"),
        torch.tensor([2277, 29937]).to("npu:0")]  # '###' can be encoded in two different ways.
