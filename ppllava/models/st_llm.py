...
class STLLMModel(Blip2Base):
    ...
    def encode_img(self, image, text=None):
        ...
        with self.maybe_autocast():
            ...
            if self.has_qformer:
                image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(f'npu:{device}' if isinstance(device, int) else device)
                ...
                if self.qformer_text_input:
                    ...
                    text_Qformer = self.tokenizer(
                        ...
                        return_tensors="pt",
                    ).to(f'npu:{query_tokens.device}' if isinstance(query_tokens.device, int) else query_tokens.device)
                    query_atts = torch.ones(query_tokens.size()[:-1], dtype=torch.long).to(f'npu:{image.device}' if isinstance(image.device, int) else image.device)
            ...
            atts_llama = torch.ones(inputs_llama.size()[:-1], dtype=torch.long).to(f'npu:{image.device}' if isinstance(image.device, int) else image.device)
        ...
        if prompts:
            ...
            for each_img_embed, each_prompt in zip(img_embeds, prompts):
                ...
                p_before_tokens = self.llama_tokenizer(
                    p_before, return_tensors="pt", add_special_tokens=False).to(f'npu:{img_embeds.device}' if isinstance(img_embeds.device, int) else img_embeds.device)
                p_after_tokens = self.llama_tokenizer(
                    p_after, return_tensors="pt", add_special_tokens=self.qformer_text_input).to(f'npu:{img_embeds.device}' if isinstance(img_embeds.device, int) else img_embeds.device)
            ...
            pad_emb = self.embed_tokens(torch.tensor(self.llama_tokenizer.pad_token_id, device=f'npu:{img_embeds.device}' if isinstance(img_embeds.device, int) else img_embeds.device))
            wrapped_embs = pad_emb.expand(len(emb_lens), max(emb_lens), -1).clone()
            wrapped_atts = torch.zeros([len(emb_lens), max(emb_lens)], dtype=torch.int, device=f'npu:{img_embeds.device}' if isinstance(img_embeds.device, int) else img_embeds.device)
    ...
    def get_residual_index(self, sample_segments, total_segments, devices):
        if hasattr(self,'residual_index'):
            ...
            frame_indices = torch.from_numpy(frame_indices).to(f'npu:{devices}' if isinstance(devices, int) else devices)
    ...
    def forward(self, samples):
        ...
        if self.pre_encoding:
            ...
            atts_img = torch.ones(img_embeds.size()[:-1], dtype=torch.long).to(f'npu:{image.device}' if isinstance(image.device, int) else image.device)
        else:
            if self.qformer_text_input:
        ...
        if not use_image and self.video_input == 'all':
            ...
            global_embeds = global_embeds.expand((-1,self.residual_size,-1,-1)).to(f'npu:{self.up_proj.weight.dtype}' if isinstance(self.up_proj.weight.dtype, int) else self.up_proj.weight.dtype)
        ...
        if not use_image and self.use_mask:
            ...
            unmask_atts_img = torch.ones(unmask_img_embeds.size()[:-1], dtype=torch.long).to(f'npu:{image.device}' if isinstance(image.device, int) else image.device)
        ...
        ).to(f'npu:{image.device}' if isinstance(image.device, int) else image.device)
        ...
        if not self.qformer_text_input:
            ...
            bos = torch.ones([batch_size, 1],
                             ...
                             device=f'npu:{to_regress_tokens.input_ids.device}' if isinstance(to_regress_tokens.input_ids.device, int) else to_regress_tokens.input_ids.device) * self.llama_tokenizer.bos_token_id
        ...
        targets = (
            torch.ones([inputs_embeds.shape[0], inputs_embeds.shape[1]],
                       dtype=torch.long).to(f'npu:{image.device}' if isinstance(image.device, int) else image.device).fill_(-100)
        )
