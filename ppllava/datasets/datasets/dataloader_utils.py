...
class MetaLoader(object):
    ...
    def build_iter(self):
        ...
        iter_order = torch.Tensor(iter_order).to(torch.device("npu")).to(f'npu:{torch.uint8}' if isinstance(torch.uint8, int) else torch.uint8)
...
def record_cuda_stream(batch):
    if isinstance(batch, torch.Tensor):
        batch.record_stream(torch_npu.npu.current_stream())
