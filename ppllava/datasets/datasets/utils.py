...
def pad_sequences_1d(sequences, dtype=torch.long, device=torch.device("cpu"), fixed_length=None):
    ...
    if isinstance(sequences[0], list):
        if "torch" in str(dtype):
            sequences = [torch.tensor(s, dtype=dtype, device=f'npu:{device}' if isinstance(device, int) else device) for s in sequences]
        else:
    ...
    if isinstance(sequences[0], torch.Tensor):
        ...
        padded_seqs = torch.zeros((len(sequences), max_length) + extra_dims, dtype=dtype, device=f'npu:{device}' if isinstance(device, int) else device)
        mask = torch.zeros((len(sequences), max_length), dtype=torch.float32, device=f'npu:{device}' if isinstance(device, int) else device)
    else:  # np
